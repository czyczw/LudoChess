...
void Game::Initial()
{
	...
	/// 开始界面
	if (!tBackBegin.loadFromFile("data/images/bg3.jpg"))
	{
		cout << "初始背景素材没有找到" << endl;
	}
	if (!tBeginButton1.loadFromFile("data/images/Begin1.png"))
	{
		cout << "开始按钮1没有找到" << endl;
	}
	if (!tBeginButton2.loadFromFile("data/images/Begin2.png"))
	{
		cout << "开始按钮2没有找到" << endl;
	}
	// 游戏选人界面
	if (!tPickBackground.loadFromFile("data/images/ol_bg.jpg"))
	{
		cout << "选将界面背景没有找到" << endl;
	}
	if (!tyuanshao.loadFromFile("data/images/re_yuanshao.jpg"))
	{
		cout << "袁绍图片没有找到" << endl;
	}
	if (!tcaoren.loadFromFile("data/images/caoren.jpg"))
	{
		cout << "曹仁图片没有找到" << endl;
	}
	if (!tsunquan.loadFromFile("data/images/re_sunquan.jpg"))
	{
		cout << "孙权图片没有找到" << endl;
	}
	/// 游戏进行界面
	if (!tBackPlay.loadFromFile("data/images/PlayBK08.jpg"))
		cout << "游戏场景素材没有找到" << endl;
	if (!tToziBut.loadFromFile("data/images/QIZI4.png"))
		cout << "骰子素材没有找到" << endl;
	if (!tQizi1.loadFromFile("data/images/QZxm.png"))
		cout << "棋子1素材没有找到" << endl;
	if (!tQizi2.loadFromFile("data/images/QZlzy.png"))
		cout << "棋子2素材没有找到" << endl;
	if (!tQizi3.loadFromFile("data/images/QZzql.png"))
		cout << "棋子3素材没有找到" << endl;
	...
	if (!t_touzi2.loadFromFile("data/images/tou2.jpg"))
		cout << "旋转棋子素材没有找到" << endl;
	if (!ttouyin.loadFromFile("data/images/touyin.png"))
		cout << "投影素材没有找到" << endl;
	if (!sbtouzi.loadFromFile("data/Audios/TZ.wav"))
	{
		cout << "骰子音乐声没有找到" << endl;
	}
	if (!sbyeah.loadFromFile("data/Audios/yeah.wav"))
	{
		cout << "棋子结束回家音乐声没有找到" << endl;
	}
	if (!bkMusic.openFromFile("data/Audios/ly68.wav"))
	{
		cout << "背景音乐声没有找到" << endl;
	}
	if (!font.loadFromFile("data/Fonts/simsun.ttc"))
		cout << "字体没有找到" << endl;
	if (!tEventBingLiang.loadFromFile("data/images/Map/bingliang.png"))
		cout << "兵粮素材没有找到" << endl;
	if (!tEventShan.loadFromFile("data/images/Map/shan.png"))
		cout << "闪素材没有找到" << endl;
	if (!tEventShanDian.loadFromFile("data/images/Map/shandian.png"))
		cout << "闪电素材没有找到" << endl;
	if (!tEventLeBu.loadFromFile("data/images/Map/lebu.png"))
		cout << "乐不思蜀素材没有找到" << endl;
	if (!tEventChiTu.loadFromFile("data/images/Map/chitu.png"))
		cout << "赤兔素材没有找到" << endl;
	if (!tEventWuZhong.loadFromFile("data/images/Map/wuzhong.png"))
		cout << "无中生有素材没有找到" << endl;
	if (!tluanji.loadFromFile("data/images/luanji.png"))
		cout << "乱击素材没有找到" << endl;
	if (!tjushou.loadFromFile("data/images/jushou.png"))
		cout << "据守素材没有找到" << endl;
	if (!tzhiheng.loadFromFile("data/images/zhiheng.png"))
		cout << "制衡素材没有找到" << endl;
	text.setFont(font);
	// 跳跃按钮
	if (!tjumpB.loadFromFile("data/images/jumpB.jpg"))
		cout << "按钮素材没有找到" << endl;
	// 结束场景
	if (!tPaiHang.loadFromFile("data/images/PH.png"))
	{
		cout << "排行榜素材没有找到" << endl;
	}
...
void Game::QiziBuxing()
{
	...
	for (int k = 0; k < 80; k++) // 数值不固定，只是让它循环一下，不一定好用，可改
	{
		...
		if (qizibuxingCount >= touziNum + jump + 1) // 棋子前进步数等于骰子数
		{
			// 如果棋子刚刚好到达终点
	...
	AITURNGOTO:																					  // AI回合先执行一遍，在处理event事件。
		...
		if (GameStart == true) // 初始界面输入模块
		{
			if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
				if (sf::Mouse::getPosition(window).x > 500 && sf::Mouse::getPosition(window).x < 600 && sf::Mouse::getPosition(window).y > 500 && sf::Mouse::getPosition(window).y < 620)
				{
		...
		if (Gamepick == true)
		{
			if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
			{
				...
				if (sf::Mouse::getPosition(window).x > yuan.x && sf::Mouse::getPosition(window).x < yuan.x + yuansize.x * yuanscale.x && sf::Mouse::getPosition(window).y > yuan.y && sf::Mouse::getPosition(window).y < yuan.y + yuansize.y * yuanscale.y)
				{
					Gamepick = false;
					GamePlay = true;
					gamemode = 1;
				}
				if (sf::Mouse::getPosition(window).x > cao.x && sf::Mouse::getPosition(window).x < cao.x + caosize.x * caoscale.x && sf::Mouse::getPosition(window).y > cao.y && sf::Mouse::getPosition(window).y < cao.y + caosize.y * caoscale.y)
				{
					Gamepick = false;
					GamePlay = true;
					gamemode = 2;
				}
				if (sf::Mouse::getPosition(window).x > sun.x && sf::Mouse::getPosition(window).x < sun.x + sunsize.x * sunscale.x && sf::Mouse::getPosition(window).y > sun.y && sf::Mouse::getPosition(window).y < sun.y + sunsize.y * sunscale.y)
				{
					Gamepick = false;
					GamePlay = true;
					gamemode = 3;
		...
		{
			sf::Vector2f jumpbut, jumpbutscale;
			sf::Vector2u jumpbutsize;
			jumpbut = sjumpB.getPosition();
			jumpbutscale = sjumpB.getScale();
			jumpbutsize = sjumpB.getTexture()->getSize();
			// 骰子时间
			...
			if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
			{
				...
				if (gamemode == 1)
				{
					...
					if (sf::Mouse::getPosition(window).x > skill.x && sf::Mouse::getPosition(window).x < skill.x + skillsize.x * skillscale.x && sf::Mouse::getPosition(window).y > skill.y && sf::Mouse::getPosition(window).y < skill.y + skillsize.y * skillscale.y && IsPlayerTurn() && skilluse == true)
					{
						this->touziTime = true;
						touzi();
						for (int i = 1; i < 4; i++)
...
void Game::Draw()
{
	...
	if (GamePlay == true)
	{
		DrawPlay();
		window.draw(sjumpB);
		sjumpB.setPosition(40, 520);
		sjumpB.setScale(0.6f, 0.5f);
		if (gamemode == 1)
		{
			window.draw(syuanshao);
...
{
	return (sf::Mouse::getPosition(window).x > 500 && sf::Mouse::getPosition(window).x < 580 && sf::Mouse::getPosition(window).y > 320 && sf::Mouse::getPosition(window).y < 380);
}
void Game::DrawDebug()
{
	if (debugMode == true)
